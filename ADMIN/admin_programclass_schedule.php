...
<body>
  ...
  if(isset($_POST['se'])){
    ...
    $startt =  date("G:i",strtotime($start_time));
    $endd =  date("G:i",strtotime($end_time));
    ...
    $check_redundant = mysqli_query($conn,"SELECT * FROM tbl_program_class WHERE Dweek = '$week[$i]' AND facility_id = '$facilityid' AND StartTime = '$startt' AND EndTime = '$endd'");
    $check_redundant_count = mysqli_num_rows($check_redundant);
    ...
    if($check_time_conflict_fac_count > 0)
    {
      ...
      {
        $start = date("G:i",strtotime($get['StartTime']));
          $end = date("G:i",strtotime($get['EndTime']));

      if($start_time == $startt && $end_time == $endd)
        {
            $counter++;
        }
    ...
    $check_time_conflict_fac = mysqli_query($conn,"SELECT * FROM tbl_program_class WHERE Dweek = '$d' AND programclass_id !='$pcid' AND facility_id = '$f'");// checking the availability of facility
      ...
      $check_time_conflict_fac_count = mysqli_num_rows($check_time_conflict_fac);
                ...
                {   
                    while($get = mysqli_fetch_array($check_time_conflict_fac))
                    {
                      $start = date("h:i:s",strtotime($get['StartTime']));
                        $end = date("h:i:s",strtotime($get['EndTime']));
                    ...
                    {
                       echo '<br/><div class="alert alert-danger">
                                      Facility is occupied. ' .$d.$s.' schedule was not updated.
                                  </div>';
                    }
                    else
                    {
